...
export function applyRelation(parent: string, child: string, path: Path) {
  ...
  const pQK = g.qkSt[parent];
  const pOrm = pQK[0];
  const pQKArg = pQK[1];
  if (!g.parents[child][pOrm]) g.parents[child][pOrm] = {};
  if (!g.parents[child][pOrm][pQKArg]) g.parents[child][pOrm][pQKArg] = [path];
  else g.parents[child][pOrm][pQKArg].push(path);
...
export function removeRelation(parent: string, child: string, path: Path) {
  ...
  if (g.childs[parent][child].length === 1) {
    delete g.parents[child][pQK[0]][pQK[1]];
    delete g.childs[parent][child];
    if (!Object.keys(g.parents[child][pQK[0]]).length) {
      delete g.parents[child][pQK[0]];
    ...
    g.parents[child][pQK[0]][pQK[1]] = g.parents[child][pQK[0]][pQK[1]].filter(
